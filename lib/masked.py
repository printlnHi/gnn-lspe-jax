...
# TODO: Adapt comment to reflect the fact that this is a fork of Haiku's BatchNorm, and only for the batchnorm
# module. Or, realiase that as I'm using Haiku the whole project is a derivate work so I just have to do a single license
# + an acknowledgment here
import types
...
masked = types.ModuleType("masked")


def mean(a: jnp.ndarray, mask: ArrayLike, axis=None, keepdims=False) -> Array:
  """Computes the mean of a, ignoring masked values.

  Args:
    a: Array to compute mean over.
    mask: Array of booleans, brodcastable to the shape of a
    axis: Axis or axes to compute mean over.
    keepdims: If true, the axes which are reduced are left in the result as
      dimensions with size one.

  Returns:
    Mean of a where mask is True
  """
  mask = jnp.broadcast_to(mask, a.shape)
  sum = jnp.sum(a * mask, axis=axis, keepdims=keepdims)
...
class BatchNorm(hk.Module):
  ...
  Excludes inputs from mean calculation if they are masked, although does normalise them.
  ...
  def __call__(
    ...
    Args:
      ...
      mask: A bolean array broadcastable to``inputs.shape[:-1]`` where True indicates relevance to the batch norm.
    ...
    mask = jnp.broadcast_to(mask, inputs.shape[:-1])
    mask = jnp.expand_dims(mask, axis=channel_index)

    if is_training or test_local_stats:
      mean = masked.mean(inputs, mask, axis, keepdims=True)
      mean_of_squares = masked.mean(
        jnp.square(inputs), mask, axis, keepdims=True)
