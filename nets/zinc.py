...
def gnn_model(net_params: Dict[str, Any]) -> NetFnType:
    ...
    def gated_gcn_net(graph: jraph.GraphsTuple, is_training: bool) -> jraph.GraphsTuple:
        """A gatedGCN model."""
        ...
        if pe_init == 'rand_walk':
            #LSPE
            raise NotImplementedError("Random walk PE nor GatedGCNLSPELayer implemented yet")
        else:
            # NoPE or LapPE
            layers = [GatedGCNLayer(hidden_dim, residual = residual, dropout=dropout) for _ in range(n_layers - 1)]
        ...
        updated_graph = jraph.GraphTuple(nodes=h, edges=e, receivers=receivers, senders=senders, n_node=n_node, n_edge=n_edge, globals=globals)
        ...
        reduction = {'sum': jnp.sum, 'max': jnp.max, 'mean': jnp.mean}.get(readout, jnp.mean) #default readout is mean
